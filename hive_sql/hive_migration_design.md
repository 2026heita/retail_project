...
当前 Reject 只覆盖日期为空和日期解析失败。数量、价格等其他技术异常尚未全部纳入 Reject 分类，因此不能描述为“已经接住所有脏数据”。
...
> 每日完整快照式 SCD2 存在日期依赖。修改历史中间日期后，如果不继续重建后续日期，后续快照可能仍然继承旧状态。
...
3. Reject 只覆盖日期为空和日期解析失败。
4. 正常 ODS 表注释仍保留早期“原始订单表”措辞，但当前职责已经是类型转换后的正常业务 ODS。
5. SCD2 SQL 逻辑已实现，但实际只验证了一个日期分区。
...
> 我没有直接把 MySQL SQL 原样搬到 Hive，而是按照离线数仓重新设计数据入口和分层。源数据先完整写入 ODS Raw，日期异常进入 Reject，正常数据进入业务 ODS，并通过三组对账保证没有静默丢数。
>
...
>
> 项目已经验证当前业务日的数据对账、事实表一致性和门禁失败路径，但实时增量、多日 SCD2 和完整调度同步仍是后续扩展方向。
