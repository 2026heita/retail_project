...
>
> 当前 Canonical 基线（1,067,371 行 Raw，DWD 805,531 行，DWD 金额 17,743,429.16）中，当前代码 Star 质量规则定义为 18 条；2010-03-04 的 canonical 历史验收发生在 STAR_018 加入前，当时质量结果为 17/17/17/0。
...
>
> 完整 run_all_hive.sh 当前共 21 个顶层执行步骤，包含 ODS Raw/Reject/正常 ODS、ODS 入仓断言、DWD 及 6 条 BLOCK 规则、DWS/ADS 指标构建、BI 经营总览日 ADS、DWS/ADS 11 条结果质量规则，以及 Star Schema 与 18 条 Star 质量规则，最后输出结果检查。DolphinScheduler 的 12 节点 JSON 已完成演示验收，但它是较早版本，目前最完整的链路仍然是 Shell 主脚本，调度 JSON 尚未同步新增的 Raw、Reject、结果门禁和星型模型。
...
> ADS 面向最终分析或展示场景，每张表对应一个清晰主题，不再承担复杂明细清洗。`ads_sales_overview_daily_hive` 为 BI Serving 主线提供日经营总览指标；`ads_sales_anomaly_daily_hive` 为独立经营异常结果，基于规则检测日度经营异常，识别 HIGH / MEDIUM 等级异常并分析主要驱动指标（ORDERS 或 AVG_ORDER_VALUE）。
...
### 12.1 历史工程完整回归（engineering_legacy_3x）

`engineering_legacy_3x` 在 `2026-04-08` 的历史工程回归已验证：
...
> 历史工程回归中的星型规则为 12/12 PASS；当前代码规则定义已扩展为 18 条。项目还使用空分区日期测试过失败路径，门禁会返回退出码 1，而不是错误地显示成功。新增 STAR_018 后应以当前版本重新执行验收，不能把历史 12/12 或 17/17 结果直接改写为 18/18。
...
> Star 历史已连续回刷并验证至 2010-03-04，覆盖 73 个交易日期；其中在 2010-03-04 对 customerid=12431 的 Belgium → Australia 真实属性变化进行了重点 SCD2 验证。完整 DWD 保留 604 个交易日期。
...
>
> 在代表性交易日中，质量阶段从约 492 秒降至约 412 秒，MR Job 从 24 降至 21；总 Job 从 43 降至 39，总耗时从约 1060 秒降至约 980 秒。但两个测试日期不同、数据规模不同，因此不是严格同数据集 benchmark，不能把百分比描述成严格性能保证。
>
...
4. SCD2 SQL 逻辑已经实现，当前已验证至 2010-03-04，覆盖 73 个交易日期，包含真实的属性变化案例。
...
13. Hive ADS 范围写入采用动态分区，一次范围式 ADS DML 覆盖 604 个交易日期；脚本仍保留独立的 DWD 前置检查和 ADS 后置验证查询。
...
16. Spring Boot 环比接口当前使用"同一 source_system 下上一可用业务日"语义，不是固定"前一日（date.minusDays(1)）"，以应对交易日期存在缺口的情况。
17. Star Schema 目前只连续验证至 2010-03-04（73 个交易日期），不是 604 个交易日期的全量历史。
...
- “Reject 已经覆盖所有脏数据。”
- “SCD2 在所有 604 个交易日期上的多版本属性变化都已完整验证。”（当前仅连续验证至 2010-03-04，73 个交易日期，并包含 customerid=12431 的真实属性变化案例）
- “CRC32 能绝对保证数据内容不变。”
- “所有性能问题都已经解决。”
...
- “SCD2 逻辑已实现，已连续验证至 2010-03-04（73 个交易日期），并通过 customerid=12431 的 Belgium → Australia 真实属性变化案例验证多版本行为。”
...
- 基于约 106 万行公开零售交易明细，覆盖 604 个交易日期，
...
- 建立 DWD 6 条、DWS/ADS 11 条和星型模型规则（历史工程验证时为 12 条，当前代码规则定义为 18 条）；BLOCK 失败时 Shell 返回非零状态，阻断下游任务。
...
- 历史工程回归（engineering_legacy_3x，2026-04-08）中源表与 ODS Raw 均为 3,202,113 行，DWD 与事实表均为 2,416,593 行、金额均为 53,230,287.48，星型质量规则全部通过；当前 Canonical 基线（1,067,371 行 Raw，DWD 805,531 行，DWD 金额 17,743,429.16）对应的当前代码 Star 规则定义为 18 条。2010-03-04 的 canonical 历史验收发生在 STAR_018 加入前，当时质量结果为 17/17/17/0。
```
...
> 确保 604 个交易日期的 total_sales、total_orders 等指标完全一致。
...
> canonical 数据验证发现交易日期存在缺口，例如 2009-12-13 的上一日 2009-12-12 无数据。
...
> 这是一个从 MySQL 分析迁移到 Hive 的学习型离线数仓项目。它不仅包含 ODS、DWD、DWS、ADS 和星型模型，还围绕原始数据保真、异常分流、数据对账、质量阻断、回刷、幂等性、SCD2、调度和性能分析做了工程化补充。当前 Shell 链路是最完整实现，DolphinScheduler 是已验收的调度演示。Star Schema 已连续验证到 2010-03-04，共 73 个交易日期，并验证了 customerid=12431 Belgium→Australia 的真实属性变化。项目已经验证核心成功和失败路径，但仍保留实时增量、604 个交易日期的 Star 全量历史验证、完整调度同步和进一步性能优化等后续空间。