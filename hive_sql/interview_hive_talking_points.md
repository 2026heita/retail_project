...
>
> 数据先从 Hive 源表 `retail` 完整落入 `ods_retail_raw_hive`。Raw 层所有业务字段先按 STRING 保存，不提前删除日期或数值异常。日期为空或解析失败的数据进入 `ods_retail_reject_hive`，日期正常且属于当前业务日的数据进入 `ods_retail_hive`。源表、Raw、正常 ODS 和 Reject 之间通过三组数量对账和 `ASSERT_TRUE` 形成 ODS 入仓门禁。
>
...
>
> 第二部分是 Hive 离线数仓。原设计直接从 `retail` 写正常 ODS，日期解析失败的数据可能在进入 ODS 前被过滤，后续检查无法发现。我把入口改成 Raw、Reject 和正常 ODS 三部分。Raw 按处理批次 `batch_dt` 保存完整源数据；Reject 保存日期为空或解析失败记录；正常 ODS 按业务日期 `dt` 保存可进入业务链路的数据。
>
...
>
> 第三部分是星型模型和工程化。用户维度采用每日完整快照式 SCD2，属性变化时关闭旧版本并生成新版本；事实表按订单日期匹配有效期内的用户版本，不是只关联当前版本。项目还有区间回刷、T+1 修正、幂等性指纹检查和历史回刷保护。
>
...
> Reject 表用于隔离不能进入正常业务链路的技术异常，同时保留原始字段、异常编码、异常原因和处理批次。当前实现主要覆盖订单日期为空和日期解析失败，不应夸大为已经覆盖全部异常类型。
...
> 每日完整快照式 SCD2 存在日期依赖。如果已经存在更晚快照，却只重跑较早的一天，后续快照可能继续继承旧状态。保护脚本会比较本次日期和最大分区日期，历史单日重跑会被阻断，并提示从该日期升序回刷到最大日期。
...
- 基于 DWD 构建 dim_user、dim_product、dim_date、dim_geo 和 fact_order，实现每日完整快照式 SCD2 用户维度及事实表有效期关联。
...
> 增加 ODS Raw，所有字段先按 STRING 保存；增加 Reject 日期异常表；正常 ODS 从 Raw 读取；增加源表、Raw、正常 ODS、Reject 三组对账和断言。

**R（结果）**

> `2026-04-08` 源表和 Raw 都是 3,202,113 行，三组差值均为 0，日期异常为 0。以后即使出现解析异常，也会被 Reject 接住而不是静默消失。
...
> 这是一个从 MySQL 分析迁移到 Hive 的学习型离线数仓项目。它不仅包含 ODS、DWD、DWS、ADS 和星型模型，还围绕原始数据保真、异常分流、数据对账、质量阻断、回刷、幂等性、SCD2、调度和性能分析做了工程化补充。当前 Shell 链路是最完整实现，DolphinScheduler 是已验收的调度演示。项目已经验证核心成功和失败路径，但仍保留实时增量、多日 SCD2、完整调度同步和深度性能优化等后续空间。