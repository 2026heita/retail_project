...
>
> 星型模型部分包含 SCD2 用户维度、商品维度、日期维度、地理维度、订单事实表和星型客户价值汇总。当前 `2026-04-08` 回归中，源表与 Raw 都是 3,202,113 行，DWD 与事实表都是 2,416,593 行、金额都是 53,230,287.48，星型模型 12 条规则全部通过。
...
>
> 完整 `run_all_hive.sh` 共 20 步，包含 ODS 入仓断言、DWD 6 条 BLOCK 规则、DWS/ADS 11 条 BLOCK/WARN 规则和星型模型 12 条规则。DolphinScheduler 的 12 节点 JSON 已完成演示验收，但它是较早版本，目前最完整的链路仍然是 Shell 主脚本，调度 JSON 尚未同步新增的 Raw、Reject、结果门禁和星型模型。
...
> 正常业务日星型 12 条规则全部通过；项目还使用空分区日期测试过失败路径，门禁会返回退出码 1，而不是错误地显示成功。
...
> SQL 逻辑实现了每日完整快照式 SCD2 语义，但当前实际数据只验证了一个分区，因此我会把“实现逻辑”和“多版本实测范围”分开说明。
...
3. Reject 当前只覆盖日期为空和日期解析失败。
4. SCD2 SQL 逻辑已经实现，但当前只实测一个分区。
...
- 当前回归中源表与 ODS Raw 均为 3,202,113 行，DWD 与事实表均为 2,416,593 行、金额均为 53,230,287.48，星型质量规则全部通过。
```
